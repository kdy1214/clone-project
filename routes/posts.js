/* 게시글 관리하는 라우터 */
// users.js코드를 이해했다면 posts.js를 이해하는 것은 크게 어렵지 않음
...
/* Routers 라우터 */
// post 컬렉션도 똑같이 findByID()를 통해 사용자 조회하듯이
// 그리고, 상황에 맞게 분기 처리
...
router.get("/post/new", isLoggedIn, (req, res) => { // 홈페이지에서 New Post 메뉴에 접속해보면
    res.render("posts/new"); // 새 게시물 등록을 위한 화면 파일 views/posts/new.ejs이 렌더링 됨
});

// users 라우터와 하나 다른 부분이 여기
router.post("/post/new", isLoggedIn, upload.single("image"), (req, res) => {
    if (req.body.content) {
        let newPost = {}; // 새로운 게시글을 담을 newPost 객체에 image, creator 등 속성을 입력해준 뒤
        if (req.file) {
            cloudinary.uploader.upload(req.file.path, result => { // cloudinary의 uploader.upload()메소드를 이용해서 받은 파일(req.file) 업로드하고
                newPost.image = result.secure_url;
                newPost.creator = req.user;
                newPost.time = new Date();
                newPost.likes = 0;
                newPost.content = req.body.content;
                return createPost(newPost, req, res); // createPost를 통해 새로운 Post를 하나 생성
            });
        } else {
            newPost.image = null;
            newPost.creator = req.user;
